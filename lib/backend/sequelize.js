import fs from "fs";
import { Sequelize, Model, DataTypes, QueryTypes, Op } from "sequelize";
import { resolve, join } from "path";
import { createHmac } from "crypto";
import { getItemById, formatCapacity } from "../helpers/catalog";
import { getDiscount, getSubtotal } from "../helpers/order";
...
export class Session extends Model {
  async restoreSession() {
    ...
    const challengedAt = this.challengedAt;
  ...
  async getDiscount() {
    const [user, cartItems, subtotal] = await Promise.all([
      this.getUser(),
      this.getCartItems(),
      this.getCartTotal(),
    ]);

    const count = cartItems.reduce((count, item) => count + item.qty, 0);
    const fixed = user ? user.get("discount") : 0;

    return getDiscount(count, subtotal, fixed);
  }
  async getCartTotal() {
  ...
  async getCartTotalWithDiscount() {
    const [cartItems, discount] = await Promise.all([
      this.getCartItems(),
      this.getDiscount(),
    ]);

    return getSubtotal(cartItems, discount);
  }
}
...
CartItem.init(
  {
    ...
    weight: {
      ...
      get() {
        ...
        const { volume, weight = 0 } = byId[this.variantId];
...
export class Order extends Model {
  ...
  static async suggestById(id) {
    const orders = await sequelize.query(
      `SELECT * FROM Orders WHERE CAST(id as CHAR(12)) LIKE '%${id}%' OR phone LIKE '%${id}%' ORDER BY createdAt DESC LIMIT 5`,
      {
...
export class LockBoxSite extends Model {
  static async suggest(query) {
    return LockBoxSite.findAll({
      include: {
        model: LockBoxSiteName,
        as: "LockBoxSiteNames",
        where: {
          name: {
...
export class LockBoxSiteName extends Model {}

LockBoxSiteName.init(
  {
    name: {
      type: DataTypes.STRING,
    },
...
LockBoxSite.hasMany(LockBoxSiteName);
LockBoxSiteName.belongsTo(LockBoxSite);
