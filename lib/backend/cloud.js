...
export async function addSitePassword(password) {
  const key = uuid();
  ...
  const payloadEntries = Object.entries({ ...entries, [key]: password }).map(
    ([key, textValue]) => ({ key, textValue })
  );
  ...
  await secretService.addVersion(
    AddVersionRequest.fromPartial({
      payloadEntries,
      description: `Set ${key}`,
      secretId: process.env.LOCKBOX_SITES_SECRET_ID,
    })
  );
