...
function formatISO(date) {
  return format(date, "yyyy-LL-dd") + "T" + format(date, "HH:mm:ssxx");
}
...
export const calculate = ensureAuthorized(async function calculate(
  ...
  const { weight, total } = items.reduce(
    (result, { weight, total }) => ({
  ...
  // 1% — страховка
  // 3.5% — комиссия юмани
  const insurance_sum = Math.floor(total * 0.045);
...
export const orderByIm = ensureAuthorized(async function order(im) {
