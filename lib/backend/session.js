...
import { Some, None, Option } from "@sniptt/monads";
import { Session, CartItem, FeedbackRequest } from "./sequelize";
import { enablePatches, produceWithPatches } from "immer";

/**
 * @typedef {import("next/server").NextRequest} NextRequest
 * @typedef {import('./sequelize').Session} Session
 *
 * @typedef {Object} SiteSession
 * @property {Object} cookie
 * @property {Array.<Object>} items
 * @property {Array.<Object>} feedbackRequests
...
async function saveSession(prevSession, newSession, patches) {
  ...
  for (const { op, path, value } of patches) {
    ...
    if (branch === "feedbackRequests") {
      if (op === "add") {
        await FeedbackRequest.createFromSite(storedSession.id, value);
      }
      if (
        op === "replace" &&
        path.length === 3 &&
        ...
        const record = await FeedbackRequest.findOne({
          where: {
            key: item.key,
          },
        });
 ...
 *
 * @returns {Promise.<Option.<SiteSession>>}
 */
...
async function upsertSession(session) {
  ...
  if (existingSession) {
    ...
    ["secret", "auth", "challengedAt", "UserId"].forEach((key) => {
      if (key in session) {
        existingSession.set(key, session[key]);
      }
    });
    ...
    ["secret", "auth", "challengedAt", "UserId"].forEach((key) => {
      if (key in session) {
        newSession.set(key, session[key]);
      }
    });
