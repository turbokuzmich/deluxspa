...
const cookieName = "deluxspa-session";
...
async function saveSession(
  ...
  for (const { op, path, value } of patches) {
    if (first(path) === "items" && processItemsUpdates) {
      if (op === "add") {
...
async function runWithSession(handler, req, res) {
  const session = await getSession(req);

  const [nextSession, patches, inversePatches] = await produceWithPatches(
    session,
    handler
  );
  ...
  await saveSession(session, nextSession, patches, inversePatches, req, res);
...
export default curry(runWithSession);
...
export async function getSession(req) {
  ...
  const restoredSession = await getRestoredSession(sessionId);
...
function getRestoredSession(sessionId) {
  return sessionId.match({
    none: Promise.resolve(SomeDefaultSession),
    async some(sessionId) {
      await sequelize;

      const storedSession = await Session.findOne({ where: { sessionId } });

      if (storedSession === null) {
        return SomeDefaultSession;
      }

      return storedSession.restoreSession().then(function (session) {
        return Some(session);
      });
    },
  });
