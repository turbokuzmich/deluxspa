...
const rootPeriod = parseInt(process.env.ROOT_PERIOD, 10);
...
export async function isAuthorized(chatId) {
  ...
  return (
    ...
    get(existingUser, "confirmed", false)
  );
}
...
export async function isRoot(chatId) {
  ...
  return (
    ...
    now - existingUser.becameRootAt.getTime() < rootPeriod
  );
}
...
export async function authorize(chatId, token, root = false) {
  ...
  const verificationSentAt = existingUser.challengedAt;

  if (Date.now() - verificationSentAt.getTime() > tokenPeriod) {
