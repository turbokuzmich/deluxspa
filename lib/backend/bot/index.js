...
import createFormatter, { escape } from "../../helpers/markdown";
                ...
                if (orderId) {
                  ...
                  if (status === "success") {
                    await bot.sendMessage(id, text, {
                      parse_mode: "MarkdownV2",
                      reply_markup: {
                        inline_keyboard: [[{ text: "Ссылка", url: infoUrl }]],
                      },
                    });
                  } else {
                    await bot.sendMessage(
...
export const notifyOfOrderPaymentStatusChange = withApi(async function (
  { bot },
  { id, status }
) {
  const chatIds = await getChatIds();
