...
import { LockBoxSite } from "../sequelize.js";
      ...
      if (initializing) {
        ...
        bot.on("callback_query", async function (message) {
            ...
            const text = Object.keys(assortment)
              .map((itemId) => {
                ...
                const varians = item.variants.list.map((variantId) => {
                  ...
                  const variantTitle = ` ${format(capacity)} ${t(
                    unitKey,
                    "ru"
                  )}`;
          ...
          if (command === "upload") {
            ...
            if (site === "neon") {
              if (locale) {
                ...
                await bot.editMessageText(
                  `Загрузите каталог на ${flag} в формате pdf размером не более 20Mb ответом на данное сообщение`,
                  { message_id, chat_id: id }
                );
                await bot.editMessageReplyMarkup(
                  {
                ...
                registerReplyExpectation(message.message, async (message) => {
                  ...
                  if (
                    ...
                    return bot.sendMessage(
                      id,
                      "Нужно загрузить pdf-файл размером не более 20Mb"
                    );
                  }
                  ...
                  try {
                    ...
                    await bot.editMessageText(`Не удалось загрузить файл`, {
                      chat_id: resultMessage.chat_id,
                      message_id: resultMessage.message_id,
                    });
                    await bot.editMessageReplyMarkup(
                      {
          ...
          if (command === "cdek") {
            ...
            if (entity === "point") {
              if (action === "view") {
                const { status, point } = await getCdekPoint(entityId);

                if (status === "success") {
                  await bot.sendMessage(id, formatPointInfo(point), {
                    parse_mode: "MarkdownV2",
                  });
                  await bot.sendLocation(
                    id,
                    get(point, ["location", "latitude"], 0),
                    get(point, ["location", "longitude"], 0)
                  );
                } else {
                  await bot.sendMessage(
                    id,
          ...
          if (command === "feedback") {
            ...
            if (action === "view") {
              ...
              if (status === "success") {
                ...
                const phone = get(feedback, "phone");
        ...
        bot.on("inline_query", async function (message) {
          ...
          if (command === "view-order") {
            ...
            if (site === "deluxspa") {
              const { status, orders } = await suggestOrders(from.id, orderId);
              ...
              if (status === "success") {
                await bot.answerInlineQuery(
                  ...
                  orders.map((order) => ({
                    ...
                    description: `На сумму ${format(
                      order.total
                    )}₽ от ${formatRU(new Date(order.createdAt))} (${
                      order.status
                    })`,
                    reply_markup: {
                      inline_keyboard: [
            ...
            if (site === "neon") {
              ...
              await bot.answerInlineQuery(
                id,
                orders.map(({ order: { id }, title, description }) => ({
                  id,
                  title,
                  description,
                  type: "article",
