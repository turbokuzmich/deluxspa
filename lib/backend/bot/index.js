    ...
    bot.onText(rootRegExp, async function (message) {
      ...
      if (await isRoot(id)) {
        ...
        replyActions[requestMessage.message_id] = async ({ text }) => {
          ...
          if (isSuccess) {
            await bot.sendMessage(id, `${name}, вам разрешен полный доступ.`);
          } else {
            await bot.sendMessage(
              id,
...
export const notifyOfFeedBack = (function () {
  const feedbackRequestsReplyListenersIds = {};

  return async function notifyOfFeedBack({ key, name, phone, email, message }) {
    const [bot, chatIds] = await Promise.all([botPromise, getChatIds()]);
