      ...
      .then(() => {
        ...
        bot.on("callback_query", async function (message) {
          ...
          if (command === "password") {
            ...
            if (type === "email") {
              ...
              //     ),
              //     async function ({ text }) {
              //       await addEmailPassword(entityId, text);
              //       await bot.sendMessage(
              //         id,
        ...
        bot.on("chosen_inline_result", async function (result) {
          ...
          if (command === "search-emails") {
            if (result_id === "new") {
              registerReplyExpectation(
                await bot.sendMessage(id, "Укажите адрес", {
                  reply_markup: {
                    force_reply: true,
                  },
                }),
                async function ({ text: email }) {
                  registerReplyExpectation(
                    ...
                    async function ({ text: password }) {
                      await addEmailPassword(email, password);
                      await bot.sendMessage(
                        id,
...
export const notifyOfPasswordSet = registerMethod(async function (bot, data) {
  console.log(data);
});
