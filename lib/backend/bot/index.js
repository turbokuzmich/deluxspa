  ...
  if (process.env.TELEGRAM_BOT_ENABLED === "1") {
    if (bot) {
      ...
      if (initializing) {
        ...
        async function viewEmailPassword(chatId, email) {
          ...
          return registerAutoDestroy(
            await bot.sendMessage(
              ...
              json2md([
                {
                  bold: escape("Вы можете скопировать пароль, нажав на него."),
                },
                {
        ...
        if (isProduction && bot.hasOpenWebHook()) {
...
export const notifyOfNewOrder = withApi(async function (api, order) {
...
export const notifyOfFeedback = withApi(
  (function () {
...
export const notifyOfPasswordSet = withApi(async function ({ bot }, data) {
  ...
  if (data.type === "site") {
    ...
    await bot.sendMessage(userId, `Пароль для ${name} успешно создан`);
  }
});

export const handleNeonBeardMessages = withApi(async function (
  { bot },
  messages
) {
  const chatIds = await getChatIds();
  ...
  for (const message of messages) {
    ...
    if (type === "neon-beard") {
      const text = get(message, "message", "");
      ...
      await Promise.all(
        chatIds.map((chatId) =>
          bot.sendMessage(
            ...
            json2md([
              {
                bold: escape("Сообщение с сайта Neon Beard"),
              },
              {
                code: escape(text),
              },
