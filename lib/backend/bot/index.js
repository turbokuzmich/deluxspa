...
import { Order, FeedbackRequest, LockBoxSite } from "../sequelize.js";
  ...
  function registrator(handler) {
    return async function (...args) {
      return bot.mapRight((botPromise) =>
        botPromise.then((bot) =>
          handler(
            { bot, registerAutoDestroy, registerReplyExpectation },
            ...args
          )
        )
      );
                ...
                json2md([
                  ...
                  },
                  {
                    p: escape(
                      [
                        `Сайт — ${site.name}`,
                        site.brief ? `Описание — ${site.brief}` : null,
                        site.url ? `URL — ${site.url}` : null,
                      ]
                        .filter(Boolean)
                        .join(". ")
                    ),
