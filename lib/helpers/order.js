...
import {
  orderStatusesKeys,
  maxDiscount,
  minSumForProgressiveDiscount,
  sumForMaximumDiscount,
} from "../../constants";
...
export function getDiscount(count, subtotal, fixed = 0) {
  if (subtotal <= minSumForProgressiveDiscount) {
    return fixed;
  }

  const progressive =
    (Math.min(sumForMaximumDiscount, subtotal) / sumForMaximumDiscount) *
    maxDiscount;
